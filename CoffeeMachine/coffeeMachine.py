...
class coffeeMachine:
    ...
    def checkState(self):
        ...
        if brewing == GPIO.HIGH and heating == GPIO.HIGH and not self.state == self.State.BREWING:
            self.chst(self.State.BREWING)
            return self.State.BREWING
        if brewing != GPIO.HIGH and heating == GPIO.HIGH and not self.state == self.State.READY:
            self.chst(self.State.READY)
            return self.State.READY
        if brewing != GPIO.HIGH and heating != GPIO.HIGH and not self.state == self.State.OFF:
            self.chst(self.State.OFF)
            return self.State.OFF
        if brewing == GPIO.HIGH and heating != GPIO.HIGH and not self.state == self.State.ERROR:
